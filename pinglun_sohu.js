...
(async () => {
  ...
  const userDataPath = 'D:\\playwright\\pw-data-sohu';
  // =================================================================
  ...
  try
  {
    ...
    // 首次运行：检查搜狐登录态，未登录则打开首页等待手动登录（登录后自动继续）
    {
      const loginPage = await context.newPage();
      await loginPage.goto('https://www.sohu.com/', { waitUntil: 'load' });
      // 搜狐登录后会有 ppinf cookie（核心登录标识）
      const hasLogin = async () => (await context.cookies()).some(c => c.name === 'ppinf');
      if (!(await hasLogin()))
      {
        console.log('⚠️ 未检测到搜狐登录态：请在弹出的窗口里手动登录搜狐，登录后会自动继续（最多等 5 分钟）。');
      ...
      console.log('✅ 已登录搜狐，开始评论任务');
      await loginPage.close();
    }

    // 循环遍历文章列表
    for (const targetUrl of sohuUrlList)
    {
      console.log(`\n===== 开始处理文章：${targetUrl} =====`);
      await checkPause();
      let page;
      try
      {
        ...
        // 先访问首页建立会话
        await page.goto('https://www.sohu.com/', { waitUntil: "load" });
        await randomSleep();
        ...
        try
        {
          // 点赞：检查是否已点赞（子元素有 like-icon 说明已点赞）
          const likeBtn = page.locator('div.wrap div.icon').filter({ has: page.locator('div.no-like-icon, div.like-icon') }).first();
          await likeBtn.waitFor({ timeout: 10000 });
          const hasLiked = await likeBtn.locator('div.like-icon').count() > 0;
          if (hasLiked)
          {
          ...
          // 收藏：检查是否已收藏（子元素有 collection-icon 说明已收藏）
          const collectBtn = page.locator('div.wrap.collection-c-1 div.icon').first();
          await collectBtn.waitFor({ timeout: 10000 });
          const hasCollected = await collectBtn.locator('div.collection-icon').count() > 0;
          if (hasCollected)
          {
        ...
        try
        {
          const commentBtn = page.locator('div.item').filter({ has: page.locator('img[src*="icon_comment.png"]') }).first();
        ...
        for (let i = 1; i <= perUrlCommentCount; i++)
        {
          ...
          try
          {
            // 定位评论输入框：搜狐用 textarea#commentEditor
            const editor = page.locator('textarea#commentEditor').first();
            ...
            // 定位并点击发布按钮
            const publishBtn = page.locator('div.submit:has-text("发布")').first();
    ...
    console.log(`\n✅ 所有${sohuUrlList.length}篇文章评论任务全部执行完毕！`);
  } catch (globalErr)
  {
